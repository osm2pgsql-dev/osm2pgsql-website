...
| t-rex  | [T-Rex](https://t-rex.bbox.earth/) tileserver | Call `themepark:plugin('t-rex'):write_config(FILENAME.toml)`. You need the [Lua toml module](https://luarocks.org/modules/lebje/toml) installed. Usually this is done with `luarocks install toml`. |
